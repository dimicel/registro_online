...
$(function() {
    ...
    dat7=dat6.then((resp)=>{
        ...
        return $.post("php/usu_transporte_verifica_curso.php", { id_nie: id_nie, curso: anno_curso_usu }, () => {});
    });
    dat8=dat7.then((resp)=>{
        if (resp == "ok"){
            ...
            alerta(mensaje, "DISCREPANCIAS ENTRE MATRÍCULA Y FORMULARIO DE TRANSPORTE","",600);
        }
    });
    ...
    .then((dialogo)=>{
            ...
            $.post("php/usu_recdatospers.php", { id_nie: id_nie }, (resp) => {
                ...
                if (resp.error == "ok") {
                    ...
                    form_mod_datos.mod_nombre.value = resp.datos.nombre;
                    form_mod_datos.mod_apellidos.value = resp.datos.apellidos;
                    form_mod_datos.mod_email.value = resp.datos.email_recuperacion;
...
function listaSolicitudes() {
    ...
    $.post("php/usu_listasolicitudes.php", { id_nie: id_nie }, function(data) {
        ...
        if (data["error"] == "noregistros") listado.innerHTML = "NO HAY REGISTRADAS SOLICITUDES";
            ...
            tabla = "<tr style='position:sticky; top:0;background-color:#228b22;width:100%;'><th style='color:white !important'>Curso</th><th style='color:white !important'>Fecha</th><th style='color:white !important'>Número de Registro</th><th style='color:white !important'>Observaciones</th></tr>";
...
function ocultaDivsSubeDocs(panel) {
    ...
    .then((dialogo)=>{
            ...
            if (panel == "foto") {
                ...
                document.getElementById("div_checkbox_pasaporte").style.display = "none";
                document.getElementById("div_certificado").style.display = "none";
                document.getElementById("div_num_ss").style.display = "inherit";
            }
            $.post("php/usu_existe_docs.php",{id_nie:id_nie,curso:anno_curso_usu,tipo_doc:panel},(resp)=>{
                if (panel=="foto"){
                    ...
                    if (resp.num_ss == 1) {
                        let img = document.getElementById('img_num_ss');
                        let txt = document.getElementById('txt_no_num_ss');
                        img.style.display = 'block'; 
                        txt.style.display = 'none';
                        ruta_num_ss='docs/' + id_nie + '/nss/nss_' +id_nie + '.jpeg?t=' + new Date().getTime();
                        img.src= ruta_num_ss;
                        ...
                        document.getElementById('txt_no_num_ss').style.display = 'block';
                        document.getElementById('btn_label_num_ss').innerText = 'Subir captura Num.SS';
                    }
                }
            },"json");
    })
