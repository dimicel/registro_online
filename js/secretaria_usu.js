                        ...
                        if(docs_exp[td] == "CONVALIDACIONES"){
                            ...
                            if (resp["docs"][td][j]["resolucion"]=="" && resp["docs"][td][j]["resolucion_con"]=="" && resp["docs"][td][j]["resolucion_min"]==""){
                                ...
                                if (resp["docs"][td][j]["resolucion"]!=""){
                                    contenido_div += "<a target='_blank' style='margin-left:5px' href='"+resp['docs'][td][j]['resolucion']+"?q="+Date.now()+"'>>Res.Centro<</a></td>";
                                }
                                if (resp["docs"][td][j]["resolucion_con"]!=""){
                                    contenido_div += "<a target='_blank' style='margin-left:5px' href='"+resp['docs'][td][j]['resolucion_con']+"?q="+Date.now()+"'>>Res.Consej.<</a></td>";
                                }
                                if (resp["docs"][td][j]["resolucion_min"]!=""){
                                    contenido_div += "<a target='_blank' style='margin-left:5px' href='"+resp['docs'][td][j]['resolucion_min']+"?q="+Date.now()+"'>>Res.Minist.<</a></td>";
                                }
                            }
                        }
...
function borraDocExp(obj) {
    cargaHTML("html/secretaria.htm", "div_borra_doc","BORRADO DE DOCUMENTO DEL EXPEDIENTE",550,500,'','',false)
    .then ((dialogo)=>{
    ...
    $.post(url,{registro:registro_adjuntos_convalid},(resp)=>{
        ...
        if(resp.error=="server") contenido += "<span class='verReg_label'>Hay un problema en sel servidor y no se han podido recuperar los documentos adjuntos.</span>";
            ...
            for(i=0;i<resp.datos.length;i++){
                contenido += "<a style='color:GREEN;font-size:0.75em; margin-right:10px;' target='_blank' href='"+resp.datos[i].ruta+"?q="+Date.now()+"'>"+resp.datos[i].descripcion+"</a>";
