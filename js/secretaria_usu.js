                ...
                if (resp["docs"][td].length > 0) {
                    ...
                    for (j = 0; j < resp["docs"][td].length; j++) {
                        ...
                        if (docs_exp[td] != "MATRICULAS" && docs_exp[td] != "PREMATRICULAS" && docs_exp[td] != "CONVALIDACIONES" && docs_exp[td] !="EXENCIÓN FORMACIÓN EN EMPRESAS (PFE)" && docs_exp[td] !="TRANSPORTE ESCOLAR") {
                            ...
                            contenido_div += "<td style='justify-content:center'><button onclick='borraDocExp(this.parentNode)' class='textoboton btn btn-danger btn-sm' data-toggle='tooltip' data-placement='right' title='Borrar documento del expediente' style='color:white;font-weight:bold; font-size:1em !important'><i class='bi bi-trash'></i></button></td>";
