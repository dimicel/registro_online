...
function obtieneDocsExpediente() {
    ...
    $.post("php/secret_usu_expedienteusu.php", { id_nie: document.getElementById("nie_exp").innerHTML, filtro: filtro_curso }, function(resp) {
        if (resp.error == "server") alerta("Error en el servidor. Inténtalo más tarde.", "Error de servidor");
            ...
            contenido_div = "<div style='height:500px !important;overflow-y:scroll;scrollbar-width: auto;' class='tablaExpediente' ><table>";
                ...
                if (resp["docs"][td].length > 0) {
                    ...
                    for (j = 0; j < resp["docs"][td].length; j++) {
                        ...
                        if (docs_exp[td] != "MATRICULAS" && docs_exp[td] != "PREMATRICULAS" && docs_exp[td] != "CONVALIDACIONES" && docs_exp[td] !="EXENCIÓN FORMACIÓN EN EMPRESAS (PFE)" ) {
                            ...
                            contenido_div += "<td><button onclick='borraDocExp(this.parentNode)' class='textoboton btn btn-danger btn-small' data-toggle='tooltip' data-placement='right' title='Borrar documento del expediente' style='color:white;font-weight:bold; font-size:1em !important'><i class='bi bi-trash'></i></button></td>";
                            contenido_div += "<td><button onclick='cambiaNomDocExp(this.parentNode)' class='textoboton btn btn-success btn-small' data-toggle='tooltip' data-placement='right' title='Cambiar nombre del documento' style='color:white;font-weight:bold; font-size:1em !important'><i class='bi bi-pencil-square'></i></i></button></td>";
                        } else {
