...
// 2. Filtro: Curso (Lógica simplificada)
if ($curso != "Todos") {
    if ($curso == "2021-2022") {
        // Tablas antiguas específicas
...
if (!$stmt_count) exit(json_encode(["error" => "Error Count: ".$mysqli->error]));
...
if (!$stmt_data) exit(json_encode(["error" => "Error Data: ".$mysqli->error, "sql" => $consulta]));
