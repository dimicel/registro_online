...
while (true){
	if ($fila < count($ausencias_avisadas)) {
		$pdf->Cell(63, 5, date("d/m/Y", strtotime($ausencias_avisadas[$fila]['fecha'])), 0, 0, 'L');
	} else {
		if($fila==0) $pdf->Cell(63, 5, 'No hay fechas', 0, 0, 'L');
		else $pdf->Cell(63, 5, '', 0, 0, 'L');
		$avis = false;
	}
	if ($fila < count($ausencias_no_avisadas)) {
		$pdf->Cell(63, 5, date("d/m/Y", strtotime($ausencias_no_avisadas[$fila]['fecha'])), 0, 0, 'C');
	} else {
		if($fila==0) $pdf->Cell(63, 5, 'No hay fechas', 0, 0, 'C');
		else  $pdf->Cell(63, 5, '', 0, 0, 'C');
		$noavis = false;
	}
	if ($fila < count($asistencias)) {
		...
		$pdf->Cell(63, 5, date("d/m/Y", strtotime($asistencias[$fila]['fecha'])).'  '.$desayuno.'   '.$comida.'    '.$cena, 0, 1, 'L');
	} else {
		if($fila==0) $pdf->Cell(63, 5, 'No hay fechas', 0, 1, 'L');
		else $pdf->Cell(63, 5, '', 0, 1, 'L');
		$asist = false;
	}
