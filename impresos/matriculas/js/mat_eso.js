...
$(document).ready(function() {
    ...
    dat2 = dat1.then((res1) => {
        ...
        return $.post("../../php/usu_existe_premat_eso.php", { id_nie: id_nie, curso: anno_ini_premat }, () => {}, "json");
    });
    dat3 = dat2.then((res2) => {
        if (res2["error"] == "ok") {
            existe_premat = true;
            curso_prematricula = res2["curso_prematricula"];
            _curso = curso_prematricula;    
    ...
    dat5=dat4.then((resp) => {
        ...
        $("#pagina_1").load("eso_html/pagina1.html?q="+Date.now().toString(), function() {
            ...
            if (existe_premat) {
                ...
                document.getElementById("prog_ling").checked=(premat_prog_ling=="No")?false:true;
...
function seleccionCurso() {
    ...
    if (_curso == "1º ESO") {
        document.getElementById("repetidor_no").checked = true;
        document.getElementById("alumno_nuevo_si").checked = true;
    ...
    if (existe_premat && curso_prematricula == _curso) {
        ...
        document.getElementById("repetidor_si").checked = true;
...
function pasaPagina(p) {
    ...
    pag = "eso_html/" + _paginas[pagina - 1][0] + ".html?q="+Date.now().toString();
...
function cargaImagen(dest){
    if (dest=="prev_foto"){
        ...
        alerta("<center><img src='../../docs/"+id_nie+"/seguro/"+id_nie+"-A.jpeg?q="+Date()+"'></center>","ANVERSO DOCUMENTO IDENTIFICACIÓN");
    }
    else if(dest=="prev_reverso_dni"){
        alerta("<center><img src='../../docs/"+id_nie+"/seguro/"+id_nie+"-R.jpeg?q="+Date()+"'></center>","REVERSO DOCUMENTO IDENTIFICACIÓN");
    }
    else if(dest=="prev_resguardo_seguro"){
        alerta("<center><img src='../../docs/"+id_nie+"/seguro/"+anno_curso+"/"+id_nie+".jpeg?q="+Date()+"'></center>","RESGUARDO SEGURO ESCOLAR");
    }
}
...
function registraMatricula() {
    ...
    //document.getElementById("consolida_premat").value=$("input:radio[name=consolida_prem]:checked").val();
    ...
    f._transporte.value = $("input:radio[name=transporte]:checked").val();
    ...
    f._prog_ling.value=(f1.prog_ling.checked)?"Si":"No";
    f.action = "php/regmat_eso.php";
