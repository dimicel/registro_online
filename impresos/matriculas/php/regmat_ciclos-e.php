...
function generaRegistro(){
    ...
    shuffle($array);
    return "iesulabto_maci-e_".date('dmY')."_".$array[0].$array[1].$array[2].$array[3].$array[4].$array[5].$array[6].$array[7];;    
}
...
$edad=intval(substr($anno_curso,0,4))-intval(substr($fecha_nac,0,4));
if($edad>=28) $mayor_28="Si";
else $mayor_28="No";
...
while ($repite_registro){
    $res=$mysqli->query("select registro from mat_ciclos where registro='$registro'");
...
$mysqli->query("insert into mat_ciclos (id_nie,
                                        registro,
                                        fecha_registro,
                                        curso,
                                        grado,
...
$YInicio+=7;
$pdf->RoundedRect($XInicio-2,$YInicio,185,12,2,'1111','','','');
$YInicio+=1;
$pdf->SetXY($XInicioRotulo,$YInicio);
$pdf->SetFont('dejavusans', 'B', 10, '', true);
