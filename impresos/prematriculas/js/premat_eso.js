...
function pasaPagina(p) {
    ...
    if (pagina != 5) {
        pag = "eso_html/pagina" + pagina + ".html?q="+Date.now().toString();
        ...
        if ($(validExec).valid()) {
            if (document.getElementById("pagina_" + pagina).innerHTML.length == 0) {
                $("#pagina_" + pagina).load(pag, function() {
                    if (String(pag).indexOf("eso_html/pagina_final.html") ==-1 ) eval(valid);
                    pasaPagina('0');
                });
            } else {
                for (i = 1; i <= 6; i++) $("#pagina_" + i).css('display', 'none');
                $("#pagina_" + pagina).css('display', 'inherit').fadeIn(500);
...
function pasaPagina(p) {
    ...
    alert(pagina+"     "+arrayPaginas[pagina].pag);return;
    if (pagina == 1) {
        $('#pagina_2').css('display', 'none');
        $('#pagina_1').fadeIn(500);
    } else {
